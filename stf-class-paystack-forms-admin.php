...
if( class_exists('stf_pff_Paystack_Admin') ) {
    class STF_Pff_Paystack_Admin extends stf_pff_Paystack_Admin
    {
        private $plugin_name;
        private $version;
        public function __construct($plugin_name, $version)
        {
            ...
            add_action('admin_menu', 'stf_pff_paystack_add_settings_page');
            add_action('admin_init', 'stf_pff_paystack_register_setting_page');

            function stf_pff_paystack_add_settings_page()
            {
                add_submenu_page('edit.php?post_type=paystack_form', 'Settings', 'Settings', 'edit_posts', basename(__FILE__), 'stf_pff_paystack_setting_page');
            }
            function stf_pff_paystack_register_setting_page()
            {
            ...
            function stf_pff_paystack_txncheck($name, $txncharge)
            {
                if ($name == $txncharge) {
                    $result = "selected";
                } else {
            ...
            function stf_pff_paystack_setting_page()
            {
                ?>
                <div class="wrap">
                            ...
                            <tr valign="top">
                                ...
                                <td>
                                    <select class="form-control" name="mode" id="parent_id">
                                        <option value="test" <?php echo stf_pff_paystack_txncheck('test', esc_attr(get_option('mode'))) ?>>Test Mode</option>
                                        <option value="live" <?php echo stf_pff_paystack_txncheck('live', esc_attr(get_option('mode'))) ?>>Live Mode</option>
                                    </select>
                            </tr>
                            <tr valign="top">
                ...
                <?php
                        ...
                        add_action('init', 'register_stf_pff_paystack');
                        function register_stf_pff_paystack()
                        {
                            $labels = array(
                        ...
                        add_filter('user_can_richedit', 'stf_pff_paystack_disable_wyswyg');

                        function stf_pff_paystack_add_view_payments($actions, $post)
                        {
                        ...
                        add_filter('page_row_actions', 'stf_pff_paystack_add_view_payments', 10, 2);


                        function stf_pff_paystack_remove_fullscreen($qtInit)
                        {
                            $qtInit['buttons'] = 'fullscreen';
                            return $qtInit;
                        }
                        function stf_pff_paystack_disable_wyswyg($default)
                        {
                            ...
                            if ($post_type == 'paystack_form') {
                                ...
                                add_action("admin_print_footer_scripts", "stf_pff_paystack_shortcode_button_script");
                                add_filter('user_can_richedit', '__return_false', 50);
                                add_action('wp_dashboard_setup', 'stf_pff_paystack_remove_dashboard_widgets');
                                remove_action('media_buttons', 'media_buttons');
                                remove_meta_box('postimagediv', 'post', 'side');
                                add_filter('quicktags_settings', 'stf_pff_paystack_remove_fullscreen');
                            }

                            return $default;
                        }
                        function stf_pff_paystack_remove_dashboard_widgets()
                        {
                        ...
                        add_filter('manage_edit-paystack_form_columns', 'stf_pff_paystack_edit_dashboard_header_columns');

                        function stf_pff_paystack_edit_dashboard_header_columns($columns)
                        {
                            $columns = array(
                        ...
                        add_action('manage_paystack_form_posts_custom_column', 'stf_pff_paystack_dashboard_table_data', 10, 2);

                        function stf_pff_paystack_dashboard_table_data($column, $post_id)
                        {
                            global $post, $wpdb;
                            $table = $wpdb->prefix . stf_pff_PAYSTACK_TABLE;
                        ...
                        add_filter('default_content', 'stf_pff_paystack_editor_content', 10, 2);

                        function stf_pff_paystack_editor_content($content, $post)
                        {
                        ...
                        /////
                        function stf_pff_paystack_editor_help_metabox($post)
                        {
                            do_meta_boxes(null, 'custom-metabox-holder', $post);
                        }
                        add_action('edit_form_after_title', 'stf_pff_paystack_editor_help_metabox');

                        function stf_pff_paystack_editor_help_metabox_details($post)
                        {
                    ...
                    <?php
                            ...
                            function stf_pff_paystack_editor_shortcode_details($post)
                            {
                                ?>
                        <p class="description">
                    ...
                    <?php
                            ...
                            add_action('add_meta_boxes', 'stf_pff_paystack_editor_add_extra_metaboxes');
                            function stf_pff_paystack_editor_add_extra_metaboxes()
                            {
                                // if ($_GET['action'] == 'edit') {
                                //     add_meta_box('stf_pff_paystack_editor_help_shortcode', 'Paste shortcode on preferred page', 'stf_pff_paystack_editor_shortcode_details', 'paystack_form', 'custom-metabox-holder');
                                // }
                                add_meta_box('stf_pff_paystack_editor_help_data', 'Help Section', 'stf_pff_paystack_editor_help_metabox_details', 'paystack_form', 'custom-metabox-holder');
                                add_meta_box('stf_pff_paystack_editor_add_form_data', 'Extra Form Description', 'stf_pff_paystack_editor_add_form_data', 'paystack_form', 'normal', 'default');
                                add_meta_box('stf_pff_paystack_editor_add_recur_data', 'Recurring Payment', 'stf_pff_paystack_editor_add_recur_data', 'paystack_form', 'side', 'default');
                                add_meta_box('stf_pff_paystack_editor_add_email_data', 'Email Receipt Settings', 'stf_pff_paystack_editor_add_email_data', 'paystack_form', 'normal', 'default');
                                add_meta_box('stf_pff_paystack_editor_add_quantity_data', 'Quantity Payment', 'stf_pff_paystack_editor_add_quantity_data', 'paystack_form', 'side', 'default');
                                add_meta_box('stf_pff_paystack_editor_add_agreement_data', 'Agreement checkbox', 'stf_pff_paystack_editor_add_agreement_data', 'paystack_form', 'side', 'default');
                                add_meta_box('stf_pff_paystack_editor_add_subaccount_data', 'Sub Account', 'stf_pff_paystack_editor_add_subaccount_data', 'paystack_form', 'side', 'default');
                                add_meta_box('stf_pff_paystack_editor_add_startdateplan_data', '*Special: Subscribe to plan after time', 'stf_pff_paystack_editor_add_startdateplan_data', 'paystack_form', 'side', 'default');
                            }


                            function stf_pff_paystack_editor_add_form_data()
                            {
                                global $post;
                            ...
                            <option value="NGN" ' . stf_pff_paystack_txncheck('NGN', $currency) . '>Nigerian Naira</option>
                            <option value="GHS" ' . stf_pff_paystack_txncheck('GHS', $currency) . '>Ghanaian Cedis</option>
                            <option value="USD" ' . stf_pff_paystack_txncheck('USD', $currency) . '>US Dollars</option>
                      </select>';
                                ...
                                echo '<select class="form-control" name="_txncharge" id="parent_id" style="width:100%;">
                                    <option value="merchant"' . stf_pff_paystack_txncheck('merchant', $txncharge) . '>No, do not add</option>
                                    <option value="customer" ' . stf_pff_paystack_txncheck('customer', $txncharge) . '>Yes, add it</option>
                                </select>
                            <br><small>This allows you include an extra charge to cushion the effect of the transaction fee. <a href="';
                                ...
                                echo '<select class="form-control" name="_loggedin" id="parent_id" style="width:100%;">
                                    <option value="no" ' . stf_pff_paystack_txncheck('no', $loggedin) . '>User must not be logged in</option>
                                    <option value="yes"' . stf_pff_paystack_txncheck('yes', $loggedin) . '>User must be logged In</option>
                                </select>';
                            ...
                            function stf_pff_paystack_editor_add_email_data()
                            {
                                global $post;
                       ...
                       <option value="no" ' . stf_pff_paystack_txncheck('no', $sendinvoice) . '>Don\'t send</option>
                       <option value="yes" ' . stf_pff_paystack_txncheck('yes', $sendinvoice) . '>Send</option>
                   </select>';
                                echo '<p>Send Email Receipt:</p>';
                                echo '<select class="form-control" name="_sendreceipt" id="parent_id" style="width:100%;">
                                <option value="no" ' . stf_pff_paystack_txncheck('no', $sendreceipt) . '>Don\'t send</option>
                                <option value="yes" ' . stf_pff_paystack_txncheck('yes', $sendreceipt) . '>Send</option>
                            </select>';
                            ...
                            function stf_pff_paystack_editor_add_recur_data()
                            {
                                global $post;
                                ...
                                <option value="no" ' . stf_pff_paystack_txncheck('no', $recur) . '>None</option>
                                <option value="optional" ' . stf_pff_paystack_txncheck('optional', $recur) . '>Optional Recurring</option>
                                <option value="plan" ' . stf_pff_paystack_txncheck('plan', $recur) . '>Paystack Plan</option>
                            </select>';
                    ...
                    <small>Plan amount must match amount on extra form description.</small>';
                            ...
                            function stf_pff_paystack_reset_stock()
                            { }
                            function stf_pff_paystack_editor_add_quantity_data()
                            {
                                global $post;
                    ...
                    <option value="no" ' . stf_pff_paystack_txncheck('no', $usequantity) . '>No</option>
                    </select>';
                                } else {
                                    echo '<select class="form-control" name="_usequantity" style="width:100%;">
                    <option value="no" ' . stf_pff_paystack_txncheck('no', $usequantity) . '>No</option>
                    <option value="yes" ' . stf_pff_paystack_txncheck('yes', $usequantity) . '>Yes</option>
                    </select>';
                                }
                                if ($usequantity == "yes") {
                    ...
                    <option value="no" ' . stf_pff_paystack_txncheck('no', $useinventory) . '>No</option>
                    <option value="yes" ' . stf_pff_paystack_txncheck('yes', $useinventory) . '>Yes</option>
                    </select>
                    ...
                    <br/>';
                            ...
                            function stf_pff_paystack_editor_add_agreement_data()
                            {
                                global $post;
                        ...
                        <option value="no" ' . stf_pff_paystack_txncheck('no', $useagreement) . '>No</option>
                        <option value="yes" ' . stf_pff_paystack_txncheck('yes', $useagreement) . '>Yes</option>
                </select>';
                                echo '<p>Agreement Page Link:</p>';
                                echo '<input type="text" name="_agreementlink" value="' . $agreementlink  . '" class="widefat" />';
                            }
                            function stf_pff_paystack_editor_add_subaccount_data()
                            {
                                global $post;
                    ...
                    <option value="account" ' . stf_pff_paystack_txncheck('account', $txnbearer) . '>Merchant (default)</option>
                    <option value="subaccount" ' . stf_pff_paystack_txncheck('subaccount', $txnbearer) . '>Sub Account</option>
                </select>';
                                echo '<p>Merchant Amount:</p>';
                                echo '<input type="text" name="_merchantamount" value="' . $merchantamount . '" class="widefat" />';
                            }
                            function stf_pff_paystack_editor_add_startdateplan_data()
                            {
                                global $post;
                            ...
                            function stf_pff_paystack_save_data($post_id, $post)
                            {
                            ...
                            add_action('save_post', 'stf_pff_paystack_save_data', 1, 2);
                        }
                    ...
                    add_action('admin_menu', 'stf_pff_paystack_register_newpage');
                    function stf_pff_paystack_register_newpage()
                    {
                        add_menu_page('paystack', 'paystack', 'administrator', 'submissions', 'stf_pff_paystack_payment_submissions');
                        remove_menu_page('submissions');
                    }

                    function stf_pff_paystack_payment_submissions()
                    {
                        $id = $_GET['form'];
                        $obj = get_post($id);
                        if ($obj->post_type == 'paystack_form') {
                            ...
                            $exampleListTable = new stf_pff_Paystack_Payments_List_Table();
                        ...
                        <div class="welcome-panel-content">
                            ...
                            <?php if ($data > 0) {
                                ...
                                <form action="<?php echo admin_url('admin-post.php'); ?>" method="post">
                                    <input type="hidden" name="action" value="stf_pff_export_excel">
                ...
                add_action('admin_post_stf_pff_export_excel', 'stf_pff_export_excel');

                function stf_pff_prep_csv_data($item)
                {
                    return '"' . str_replace('"', '""', $item) . '"';
                }

                function stf_pff_export_excel()
                {
                    global $wpdb;
                    ...
                    if (count($alldbdata) > 0) {
                        ...
                        if (array_key_exists("0", $new)) {
                            foreach ($new as $key => $item) {
                                $csv_output .= stf_pff_prep_csv_data($item->display_name) . ",";
                            }
                        } else {
                            if (count($new) > 0) {
                                foreach ($new as $key => $item) {
                                    $csv_output .= stf_pff_prep_csv_data($key) . ",";
                                }
                            }
                        }
                        $csv_output .= "\n";
                        ...
                        foreach ($alldbdata as $key => $dbdata) {
                            ...
                            $csv_output .= stf_pff_prep_csv_data($newkey) . ",";
                            $csv_output .= stf_pff_prep_csv_data($dbdata->email) . ",";
                            $csv_output .= stf_pff_prep_csv_data($dbdata->cause_id) . ",";
                            $csv_output .= stf_pff_prep_csv_data($dbdata->cause_title) . ",";
                            $csv_output .= stf_pff_prep_csv_data($currency . ' ' . $dbdata->amount) . ",";
                            $csv_output .= stf_pff_prep_csv_data(substr($dbdata->paid_at, 0, 10)) . ",";
                            $csv_output .= stf_pff_prep_csv_data($txn_code) . ",";
                            $new = json_decode($dbdata->metadata);
                            $text = '';
                            if (array_key_exists("0", $new)) {
                                foreach ($new as $key => $item) {
                                    $csv_output .= stf_pff_prep_csv_data($item->value) . ",";
                                }
                            } else {
                                if (count($new) > 0) {
                                    foreach ($new as $key => $item) {
                                        $csv_output .= stf_pff_prep_csv_data($item) . ",";
                                    }
                                }
                            }
                            $csv_output .= "\n";
                ...
                class STF_Pff_Paystack_Wp_List_Table
                {
                ...
                class STF_Pff_Paystack_Payments_List_Table extends WP_List_Table
                {
                    public function prepare_items()
                    {
                        ...
                        foreach ($alldbdata as $key => $dbdata) {
                            ...
                            $data[] = array(
                                ...
                                'txn_code' => $txn_code,
                                'metadata' => stf_format_data($dbdata->metadata),
